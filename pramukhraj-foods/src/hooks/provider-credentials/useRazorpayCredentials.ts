...
export function useRazorpayCredentials() {
  ...
  const [data, setData] = useState<ProviderCredentialResponse<RazorpayCredentials> | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    try {
      const response = await providerCredentialsApi.getRazorpay(PROVIDER_KEYS.razorpay, controller.signal)
  ...
  const save = useCallback(async (credentials: RazorpayCredentials): Promise<ApiResponse<string>> => {
    setIsSaving(true)
    try {
      const response = isConfigured
        ? await providerCredentialsApi.update(PROVIDER_KEYS.razorpay, { credentials, isActive: true })
        : await providerCredentialsApi.create({
            providerKey: PROVIDER_KEYS.razorpay,
            credentials,
            isActive: true,
          })
