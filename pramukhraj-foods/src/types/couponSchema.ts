...
export const couponFormSchema = z
  .object({
    ...
    minimumOrderAmount: z
      .number({ error: "Enter a valid minimum order." })
      .min(0, "Minimum order cannot be negative."),
  maximumDiscountAmount: z
      .number({ error: "Enter a valid maximum discount." })
      .nullable(),
    ...
    productIds: z.array(
      z.string().regex(guidPattern, "A selected product is invalid.")
    ),
    categoryIds: z.array(
      z.string().regex(guidPattern, "A selected category is invalid.")
    ),
    totalUsageLimit: z
      .number()
      .int()
      .positive("Total usage limit must be greater than 0.")
      .nullable(),
  ...
  .superRefine((data, context) => {
    ...
    if (data.discountType === "FreeShipping" && data.discountValue !== 0) {
      context.addIssue({
        code: "custom",
        path: ["discountValue"],
        message: "Free shipping must have a discount value of 0.",
      });
    }
    if (
      data.discountType === "Percentage" &&
    ...
    if (
      data.perCustomerUsageLimit !== null &&
      data.totalUsageLimit !== null &&
      data.perCustomerUsageLimit > data.totalUsageLimit
    ) {
      context.addIssue({
        code: "custom",
        path: ["perCustomerUsageLimit"],
        message: "Per-customer limit cannot exceed the total limit.",
      });
    }
    ...
    if (
      data.applicationScope === "AllProducts" &&
      (data.productIds.length > 0 || data.categoryIds.length > 0)
    ) {
      context.addIssue({
        code: "custom",
        path: ["applicationScope"],
        message: "All-products coupons cannot contain scope selections.",
      });
    }
    if (data.applicationScope === "SpecificProducts") {
      ...
      if (data.categoryIds.length > 0)
        context.addIssue({
          code: "custom",
          path: ["categoryIds"],
          message: "Clear category selections.",
        });
    }
    if (data.applicationScope === "SpecificCategories") {
      ...
      if (data.productIds.length > 0)
        context.addIssue({
          code: "custom",
          path: ["productIds"],
          message: "Clear product selections.",
        });
    }
  });
...
export const DEFAULT_COUPON_VALUES: CouponFormValues = {
  ...
  startOn: toDateTimeLocal(start),
  endOn: toDateTimeLocal(end),
  isActive: true,
};
...
export function mapCouponFormToRequest(
