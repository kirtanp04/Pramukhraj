  ...
  return (
    <div className="flex min-h-screen bg-teal-deep text-ivory">
      <div className="hidden w-1/2 flex-col justify-between bg-teal p-12 lg:flex">
        ...
        <div>
          <h1 className="font-display text-4xl leading-tight">Commerce Console</h1>
          <p className="mt-4 max-w-md text-sm text-ivory/70">
            Manage products, orders, content and access for PramukhRaj Foods from a single,
            role-aware admin workspace.
          </p>
        </div>
        <p className="text-xs text-ivory/50">© 2026 PramukhRaj Foods Pvt. Ltd. Internal use only.</p>
      </div>

      <div className="flex flex-1 items-center justify-center px-4 py-12">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
          <div className="mb-8 flex justify-center lg:hidden"><Logo className="text-ivory" /></div>
          <div className="rounded-card border border-ivory/10 bg-ivory/[0.04] p-6 backdrop-blur">
            ...
            <p className="mb-6 text-sm text-ivory/60">Role-based access — only registered admins may enter.</p>

            <form onSubmit={submit} className="space-y-4">
              <label className="block">
                <span className="mb-1 flex items-center gap-1.5 text-xs text-ivory/60"><Mail size={13} /> Email</span>
                <input
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  type="email"
                  required
                  placeholder="you@pramukhraj.com"
              ...
              <label className="block">
                <span className="mb-1 flex items-center gap-1.5 text-xs text-ivory/60"><Lock size={13} /> Password</span>
                <div className="relative">
                  ...
                  />
                  <button type="button" onClick={() => setShowPassword((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-ivory/50">
              ...
              {loginError && (
                <p className="rounded-lg bg-oxblood/20 px-3 py-2 text-xs text-oxblood-deep text-oxblood">{loginError}</p>
              )}
            ...
            <div className="mt-6 border-t border-ivory/10 pt-5">
              <p className="mb-2 text-xs uppercase tracking-wide text-ivory/40">Try a demo role</p>
              ...
              <p className="mt-3 text-[11px] text-ivory/40">Demo password for every account: <span className="font-mono text-ivory/60">{DEMO_PASSWORD}</span></p>
            </div>
          </div>
        </motion.div>
      </div>
