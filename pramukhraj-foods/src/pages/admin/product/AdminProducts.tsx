...
type ProductColumns = Parameters<typeof DataTable<AdminProductList>>[0]['columns']
...
function parseVariantPrice(value: string, index: number): VariantPriceDisplay {
  ...
  if (separatorIndex <= 0 || separatorIndex === value.length - 1) {
    return { key: `${value}-${index}`, price: value || 'Unavailable', weight: 'Variant' }
  }
...
export function AdminProducts() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<AdminProductList | null>(null)
  ...
  const columns = useMemo<ProductColumns>(() => [
        ...
        return prices.length > 0 ? (
          <div className="min-w-20 space-y-1">
            {prices.map(parseVariantPrice).map((variant) => (
              <div key={variant.key} className="flex items-center justify-start gap-3 text-xs">
                <span className="font-mono font-semibold text-oxblood">{variant.price}</span>
                <span className="font-medium text-ink-soft">{variant.weight}</span>
              </div>
            ))}
          </div>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Badge variant={stock === 0 ? 'oxblood' : stock < 15 ? 'turmeric' : 'teal'}>
      ...
      cell: ({ row }) => {
        ...
        return labels.length > 0 ? (
          <div className="flex max-w-52 flex-wrap gap-1">
            {labels.map((label) => <Badge key={label} variant="soft">{label}</Badge>)}
          </div>
        ) : <span className="text-xs text-ink-soft">—</span>
      },
    },
    {
      ...
      cell: ({ row }) => (
        <Badge variant={row.original.isActive ? 'teal' : 'outline'}>
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-1">
          <a
            ...
            aria-label={`View ${row.original.name} on storefront`}
          >
    ...
    <div>
      ...
      {listError ? (
        ...
        <DataTable
          ...
          emptyMessage={page > 1 ? 'No products found on this page.' : 'No products found.'}
