...
export const EMAIL_TEMPLATE_STARTERS: readonly EmailTemplateStarter[] = [
  ...
  starter(
    { key: 'INVOICE', name: 'Order Receipt / Bill of Supply', description: 'Provides the official bill of supply / order receipt for a completed order.', category: EMAIL_TEMPLATE_CATEGORY.Order, subject: 'Order Receipt for order {{order_number}}', variables: ['customer_name', 'order_number', 'invoice_number', 'invoice_url'], plainTextContent: 'Hi {{customer_name}}, your order receipt for order {{order_number}} is ready: {{invoice_url}}.' },
    { title: 'Your order receipt is ready', preheader: 'Order receipt / Bill of Supply for order {{order_number}} is available.', greeting: 'Hi {{customer_name}},', message: 'Your official Bill of Supply / Order Receipt for order {{order_number}} is ready to view or download. All prices are inclusive of all taxes.', highlight: 'Order #{{order_number}}', action: { label: 'View receipt', url: '{{invoice_url}}' } },
  ),
  starter(
